...
describe('RecordFetcherRepository', () => {
  ...
  describe('getAllRecords', () => {
    it('특정 참가자의 모든 기록을 성공적으로 가져와야 한다', async () => {
      ...
      // Assert
      expect(mockFetcher.get).toHaveBeenCalledWith(`/api/participants/${participantId}/records`);
  ...
  describe('getRecordById', () => {
    it('특정 기록 정보를 성공적으로 가져와야 한다', async () => {
      ...
      // Assert
      expect(mockFetcher.get).toHaveBeenCalledWith(`/api/records/${recordId}`);
  ...
  describe('getTopRecords', () => {
    it('특정 디비전의 최고 기록을 성공적으로 가져와야 한다', async () => {
      ...
      // Assert
      expect(mockFetcher.get).toHaveBeenCalledWith(`/api/divisions/${divisionId}/records/top`);
      expect(parseRecordDto).toHaveBeenCalledTimes(2);
      expect(result).toEqual([mockRecord, mockRecord]);
    });
  ...
  describe('createRecord', () => {
    it('새 기록을 성공적으로 생성해야 한다', async () => {
      ...
      // Assert
      expect(mockAuthFetcher.post).toHaveBeenCalledWith(`/api/participants/${participantId}/records`, {
        body: newRecord,
      });
      expect(parseRecordDto).toHaveBeenCalledWith(mockRecordDto);
      expect(result).toEqual(mockRecord);
    ...
    it('유효하지 않은 데이터로 생성 시 에러를 처리해야 한다', async () => {
      ...
      const invalidRecord = {
        ...
        note: 'Note',
      } as Pick<Record, 'value' | 'source' | 'note'>;

      const validationError = new Error('유효하지 않은 기록 데이터입니다');
      mockAuthFetcher.post = vi.fn().mockRejectedValue(validationError);

      // Act & Assert
      await expect(recordRepository.createRecord(participantId, invalidRecord)).rejects.toThrow('유효하지 않은 기록 데이터입니다');
    });
  });
  ...
  describe('updateRecordNote', () => {
    it('기록 노트를 성공적으로 업데이트해야 한다', async () => {
      ...
      // Assert
      expect(mockAuthFetcher.patch).toHaveBeenCalledWith(`/api/records/${recordId}/note`, {
  ...
  describe('updateRecordStatus', () => {
    it('기록 상태를 성공적으로 업데이트해야 한다', async () => {
      ...
      // Assert
      expect(mockAuthFetcher.patch).toHaveBeenCalledWith(`/api/records/${recordId}/status`, {
    ...
    it('유효하지 않은 상태로 업데이트 시 에러를 처리해야 한다', async () => {
      ...
      // Act & Assert
      await expect(recordRepository.updateRecordStatus(recordId, invalidStatus)).rejects.toThrow('유효하지 않은 상태입니다');
    });
