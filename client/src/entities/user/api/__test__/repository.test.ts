...
describe("UserRepositoryImpl", () => {
  ...
  describe("getAllUsers", () => {
    it("모든 사용자 목록을 반환해야 한다", async () => {
      ...
      // Assert
      expect(mockAuthenticatedFetcher.get).toHaveBeenCalledWith("/api/actors");
  ...
  describe("getCurrentUser", () => {
    it("현재 사용자 정보를 반환해야 한다", async () => {
      ...
      // Assert
      expect(mockAuthenticatedFetcher.get).toHaveBeenCalledWith("/api/actors/whoami");
  ...
  describe("registerUser", () => {
    it("새 사용자를 등록하고 사용자 정보를 반환해야 한다", async () => {
      ...
      // Assert
      expect(mockPublicFetcher.post).toHaveBeenCalledWith("/api/actors/register", {
        body: {
          name: "New User",
  ...
  describe("loginUser", () => {
    it("로그인 성공 시 사용자 정보와 세션 키를 반환해야 한다", async () => {
      ...
      // Assert
      expect(mockPublicFetcher.post).toHaveBeenCalledWith("/api/actors/login", {
        body: {
          username: "testuser",
          password: "password123",
        },
      });
      expect(mockPublicFetcher.get).toHaveBeenCalledWith("/api/actors/whoami", {
        headers: {
    ...
    it("세션 키는 받았지만 사용자 정보 조회 실패 시 에러를 발생시켜야 한다", async () => {
      ...
      mockPublicFetcher.get = vi.fn().mockRejectedValue(new Error("사용자 정보 조회 실패"));
  ...
  describe("logoutUser", () => {
    it("로그아웃 요청을 수행해야 한다", async () => {
      ...
      // Assert
      expect(mockAuthenticatedFetcher.post).toHaveBeenCalledWith("/api/actors/logout");
    });
  ...
  describe("updateUserRoles", () => {
    it("사용자 권한을 업데이트하고 업데이트된 사용자 정보를 반환해야 한다", async () => {
      ...
      // Assert
      expect(mockAuthenticatedFetcher.post).toHaveBeenCalledWith(`/api/actors/${userId}/roles`, {
        body: { roles: newRoles },
      });
  ...
  describe("deleteUser", () => {
    it("사용자를 삭제해야 한다", async () => {
      ...
      // Assert
      expect(mockAuthenticatedFetcher.delete).toHaveBeenCalledWith(`/api/actors/${userId}`);
    });
