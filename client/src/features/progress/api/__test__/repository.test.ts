...
describe('ProgressFetcherRepository', () => {
  ...
  describe('getProgress', () => {
    it('특정 디비전의 진행 상태를 성공적으로 가져와야 한다', async () => {
      ...
      // Assert
      expect(mockFetcher.get).toHaveBeenCalledWith(`/api/divisions/${divisionId}/progress`);
  ...
  describe('openProgressDivision', () => {
    it('디비전 진행을 성공적으로 시작해야 한다', async () => {
      ...
      // Assert
      expect(mockAuthFetcher.post).toHaveBeenCalledWith(`/api/divisions/${divisionId}/progress/open`);
    });
  ...
  describe('closeProgressDivision', () => {
    it('디비전 진행을 성공적으로 종료해야 한다', async () => {
      ...
      // Assert
      expect(mockAuthFetcher.post).toHaveBeenCalledWith(`/api/divisions/${divisionId}/progress/close`);
    });
    ...
    it('진행 중이지 않은 디비전에 대한 요청 시 에러를 처리해야 한다', async () => {
      ...
      // Act & Assert
      await expect(progressRepository.closeProgressDivision(divisionId)).rejects.toThrow('진행 중이지 않은 디비전입니다');
    });
  });
  ...
  describe('resetProgressDivision', () => {
    it('디비전 진행을 성공적으로 리셋해야 한다', async () => {
      ...
      // Assert
      expect(mockAuthFetcher.post).toHaveBeenCalledWith(`/api/divisions/${divisionId}/progress/reset`);
    });
    ...
    it('진행 중인 디비전에 대한 리셋 요청 시 에러를 처리해야 한다', async () => {
      ...
      // Act & Assert
      await expect(progressRepository.resetProgressDivision(divisionId)).rejects.toThrow('진행 중인 디비전은 리셋할 수 없습니다');
    });
  ...
  describe('setCurrentRunner', () => {
    it('현재 주자를 성공적으로 설정해야 한다', async () => {
      ...
      // Assert
      expect(mockAuthFetcher.patch).toHaveBeenCalledWith(`/api/divisions/${divisionId}/progress/runner`, {
        body: { participantId },
      });
    });
    ...
    it('존재하지 않는 참가자에 대한 요청 시 에러를 처리해야 한다', async () => {
      ...
      // Act & Assert
      await expect(progressRepository.setCurrentRunner(divisionId, participantId)).rejects.toThrow('참가자를 찾을 수 없습니다');
    });
    ...
    it('권한 없음 에러를 처리해야 한다', async () => {
      ...
      // Act & Assert
      await expect(progressRepository.setCurrentRunner(divisionId, participantId)).rejects.toThrow('주자 설정 권한이 없습니다');
    });
  });
  ...
  describe('postponeCurrentRunner', () => {
    it('현재 주자를 성공적으로 연기해야 한다', async () => {
      ...
      // Assert
      expect(mockAuthFetcher.post).toHaveBeenCalledWith(`/api/divisions/${divisionId}/progress/runner/postpone`);
    });
    ...
    it('현재 주자가 없는 경우 에러를 처리해야 한다', async () => {
      ...
      // Act & Assert
      await expect(progressRepository.postponeCurrentRunner(divisionId)).rejects.toThrow('현재 주자가 설정되지 않았습니다');
    });
  ...
  describe('getOrder', () => {
    it('디비전의 주자 순서를 성공적으로 가져와야 한다', async () => {
      ...
      // Assert
      expect(mockFetcher.get).toHaveBeenCalledWith(`/api/divisions/${divisionId}/progress/order`);
  ...
  describe('changeOrder', () => {
    it('주자 순서를 성공적으로 변경해야 한다', async () => {
      ...
      // Assert
      expect(mockAuthFetcher.patch).toHaveBeenCalledWith(`/api/divisions/${divisionId}/progress/order`, {
    ...
    it('유효하지 않은 순서로 변경 시 에러를 처리해야 한다', async () => {
      ...
      // Act & Assert
      await expect(progressRepository.changeOrder(divisionId, participantId, order)).rejects.toThrow('유효하지 않은 순서입니다');
    });
    ...
    it('존재하지 않는 참가자에 대한 요청 시 에러를 처리해야 한다', async () => {
      ...
      // Act & Assert
      await expect(progressRepository.changeOrder(divisionId, participantId, order)).rejects.toThrow('참가자를 찾을 수 없습니다');
    });
    ...
    it('권한 없음 에러를 처리해야 한다', async () => {
      ...
      // Act & Assert
      await expect(progressRepository.changeOrder(divisionId, participantId, order)).rejects.toThrow('순서 변경 권한이 없습니다');
    });
  });
