...
describe('createAuthService', () => {
  ...
  describe('login', () => {
    it('성공적인 로그인 시 사용자 정보를 설정하고 세션을 저장해야 한다', async () => {
      ...
      mockUserRepository.loginUser = vi.fn().mockResolvedValue(loginResult);
  ...
  describe('register', () => {
    ...
    it('회원가입 실패 시 에러를 발생시키고 상태를 초기화해야 한다', async () => {
      ...
      // Act & Assert
      await expect(authService.register('Test User', 'testuser', 'password')).rejects.toThrow('이미 존재하는 사용자입니다.');
      expect(useAuthStore.getState().user).toBeNull();
      expect(useAuthStore.getState().isAuthenticated).toBe(false);
      expect(localStorageMock.removeItem).toHaveBeenCalledWith('user_session');
    });
