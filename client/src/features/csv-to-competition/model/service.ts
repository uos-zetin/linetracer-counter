...
export function createCsvService(dependencies: CsvServiceDependencies) {
   ...
   * 대회 생성
   */
  const createCompetition = async (competitionName: string): Promise<string> => {
    setStep('creating_competition', 10);
    updateProgress(0, `대회 "${competitionName}" 생성 중...`);

    const competitionData: CompetitionForm = {
      name: competitionName,
      description: `CSV로 생성된 대회 - ${new Date().toLocaleDateString()}`,
    };
    ...
    for (const group of groupedData) {
      ...
      const result = await retryWithBackoff(
        async () => {
          ...
          // TODO: 실제 배치 API 엔드포인트로 대체 필요
          // 현재는 개별 생성으로 처리
          await Promise.all(
            participantData.map(participant =>
              participantRepository.createParticipant(divisionId, participant)
            )
          );
          return true; // 성공 표시
        },
      ...
      if (!result.success) {
        throw new Error(
          `부문 "${group.divisionName}" 참가자 등록 실패: ${result.error}`
        );
      }
      ...
      updateProgress(
        (completedGroups / totalGroups) * 100,
        `부문 "${group.divisionName}" 참가자 ${group.participants.length}명 등록 완료`
      );
    }
   ...
   */
  const processCSV = async (file: File): Promise<CsvProcessResult> => {
    ...
    try {
      ...
      // 3. 대회 생성
      const competitionId = await createCompetition(competitionName);
      ...
      const divisionIdMap = await createDivisions(competitionId, divisionNames);
      ...
      // 5. 참가자 생성
      await createParticipants(groupedData, divisionIdMap);
      ...
      groupedData.forEach(group => {
        group.participants.forEach(participant => {
          createdEntities.push({
            type: 'participant',
            id: `${participant.name}-${group.divisionName}`, // 임시 ID
            name: participant.name,
            createdAt: new Date(),
          });
        });
