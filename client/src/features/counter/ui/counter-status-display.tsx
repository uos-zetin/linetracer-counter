    ...
    <div className={`p-4 border rounded-lg ${getBgColor()} ${className}`}>
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold">{counterName || `Counter ${counterId}`}</h3>
        ...
        <div className="text-2xl font-mono font-bold">{formatElapsedMs(elapsedMs).toString()}</div>

        <div className="text-xs text-gray-500 space-y-1">
          {startedAt && <div>Started: {new Date(startedAt).toLocaleTimeString()}</div>}
          {stoppedAt && <div>Stopped: {new Date(stoppedAt).toLocaleTimeString()}</div>}
        </div>
      </div>
    </div>
  );
